...
export const getDashboardMetrics = async (req: Request, res: Response) => {
  try {
    ...
    const [usuariosActivos, especialistas, consultasHoy, totalIngresos, promedioRating] = await Promise.all([
      prisma.user.count({
        where: {
          status: 'Activo',
        }
    ...
    ].sort((a, b) => new Date(b.fechaOriginal).getTime() - new Date(a.fechaOriginal).getTime())
     .slice(0, 5)
     .map(item => {
    ...
    const demografiaGenero = {
      mujeres: genderStats.find(g => g.gender.toLowerCase().includes('mujer') || g.gender.toLowerCase().includes('femenino'))?._count.gender || 0,
      hombres: genderStats.find(g => g.gender.toLowerCase().includes('hombre') || g.gender.toLowerCase().includes('masculino'))?._count.gender || 0
    };
