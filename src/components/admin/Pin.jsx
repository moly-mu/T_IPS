...
import SpecialistsSection from './specialist/SpecialistsSection';
    ...
    <div className="flex h-screen">
      <div className="w-64 bg-white border-r fixed top-0 left-0 z-50 h-full flex flex-col">
        <div className="h-[var(--h-nav)] p-4 flex items-center gap-2 cursor-pointer">
          ...
          <div>
            <Link to="/" className="text-sm font-bold text-gray-800">IPS IUDC</Link>
          </div>
        </div>
...
const RecentActivity = () => {
  const activities = [
    { id: 1, action: "Nueva consulta", user: "María González", time: "Hace 5 min", type: "consultation" },
    { id: 2, action: "Especialista registrado", user: "Dr. Carlos Ruiz", time: "Hace 15 min", type: "specialist" },
    { id: 3, action: "Pago completado", user: "Ana Rodríguez", time: "Hace 30 min", type: "payment" },
    { id: 4, action: "Consulta finalizada", user: "Dr. Luis Pérez", time: "Hace 1 hora", type: "consultation" }
  ];

  const getIcon = (type) => {
    switch (type) {
      case 'consultation': return <Calendar className="w-4 h-4 text-blue-600" />;
      case 'specialist': return <UserCheck className="w-4 h-4 text-green-600" />;
      case 'payment': return <Activity className="w-4 h-4 text-purple-600" />;
