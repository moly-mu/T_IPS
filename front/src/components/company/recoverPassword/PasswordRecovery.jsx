...
const PasswordRecovery = () => {
  ...
  // inicializa refs para los inputs del codigo
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    setAlert({ type: 'error', message: 'Mensaje de error aquí' });
    ...
    try {
      await new Promise((resolve, reject) => {
        setTimeout(() => {
          // Simular exito o error aleatoriamente para testing
          if (Math.random() > 0.1) {
            resolve();
          } else {
  ...
  const handleCodeChange = (index, value) => {
    ...
    // mover al siguiente input si se ingreso un numero
    if (value && index < 5) {
  ...
  const handleCodePaste = (e) => {
    ...
    if (digits.length > 0) {
      ...
      // enfoca el siguiente input vacio o el ultimo si todos estan llenos
  ...
  const handleGmailRedirect = () => {
    // el dominio del email para abrir el cliente correcto
    ...
    switch (domain) {
      ...
      case 'yahoo.com':
        ...
        break;
      default:
        //intentar abrir Gmail o el cliente de correo predeterminado
        mailUrl = 'https://mail.google.com';
    }
