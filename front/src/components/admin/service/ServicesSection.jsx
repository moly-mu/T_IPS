...
const ServicesSection = () => {
  ...
  const manejarSubmit = () => {
    ...
    if (modalTipo === "crear") {
      ...
      axios
        .put(`http://localhost:3000/admin/specialty/${servicioSeleccionado.id}`, payload)
        .then((res) => {
          setServicios(
            servicios.map((s) => (s.id === servicioSeleccionado.id ? res.data : s))
          );
          cerrarModal();
        })
        .catch((err) => console.error("Error al actualizar especialidad:", err));
    }
  };
  ...
  const cambiarEstado = (id) => {
    ...
    axios
      .put(`http://localhost:3000/admin/specialty/${id}`, {
        ...servicio,
        status: nuevoEstado,
      })
      .then((res) => {
  ...
  const serviciosActivos = servicios.filter(s => s.status === "Activo").length;
  const totalConsultas = 120;
  const ingresoTotal = servicios.reduce((acc, s) => acc + s.price, 0);
  const porcentajeActivos = totalServicios > 0
        ...
        <div className="bg-white rounded-lg p-6 border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-green-600 mt-1">{porcentajeActivos}% del total</p>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg w-full max-w-md">
            <h2 className="text-xl font-semibold mb-4">
              {modalTipo === "crear" ? "Crear Especialidad" : "Editar Especialidad"}
            </h2>
